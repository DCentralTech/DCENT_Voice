...
def test_fresh_profile_launch_never_pipes_the_apps_streams() -> None:
    ...
    Phase 2 must therefore never hand the app a pipe.

    The streams themselves are chosen per-OS. Windows keeps them at None so
    Python does not set STARTF_USESTDHANDLES and DETACHED_PROCESS can deny the
    child a console; on POSIX None means *inherit*, which would hand the child
    this harness's own pipe, so they point at /dev/null instead.
    """
    source = FRESH_SCRIPT.read_text(encoding="utf-8")
    ...
    assert "stdin=_DETACHED_STDIO" in launch
    assert "stdout=_DETACHED_STDIO" in launch
    assert "stderr=_DETACHED_STDIO" in launch
    assert "creationflags=_DETACHED_CREATION_FLAGS" in launch


def test_detached_stdio_never_inherits_and_never_pipes(smoke) -> None:
    """Windows needs None; POSIX must redirect or it inherits the harness's pipe."""
    if os.name == "nt":
        assert smoke._DETACHED_STDIO is None
    else:
        assert smoke._DETACHED_STDIO == subprocess.DEVNULL
    assert smoke._DETACHED_STDIO != subprocess.PIPE
...
@pytest.mark.skipif(
    os.name != "nt",
    reason="sys.stdout is None only on a Windows windowed build; POSIX always reports a handle",
)
...
@pytest.mark.skipif(os.name == "nt", reason="POSIX has no 'no stdout' process state")
def test_stdio_assertion_accepts_a_handle_on_posix(smoke, tmp_path, capsys) -> None:
    """A .desktop launch still gives the process descriptors, so handle is correct.

    The launch points them at /dev/null in a new session; requiring stdout=none
    here would make the POSIX package smokes unpassable by construction.
    """
    logs = tmp_path / "config" / "logs"
    logs.mkdir(parents=True)
    (logs / "startup.log").write_text(
    ...
    assert "detached session" in capsys.readouterr().out
