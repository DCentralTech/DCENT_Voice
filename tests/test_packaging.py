...
@pytest.mark.skipif(os.name != "nt", reason="runs write_sha256.ps1 through native powershell.exe")
...
def test_windows_release_checksum_wiring_is_declared_in_the_scripts() -> None:
    """Checksums must be taken after signing, on every OS that reads these files.

    Reading the scripts needs no Windows: keeping this out of the powershell.exe
    test above means Linux and macOS still catch a release pipeline that
    checksums a binary before Authenticode changes its bytes.
    """
