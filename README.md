...
wake/toggle modes, and ADE/DVAP. **Windows and Linux are supported today; macOS is
an incoming target.** Expect rough edges; please file issues.
...
DCENT_Voice is built cross-platform: capture, the speech engine, the dictation
pipeline, text injection, tray, and overlay all have per-OS implementations, and
CI runs the full test suite on Windows, Linux, and macOS.

| Platform | Status | How you install it |
|---|---|---|
| **Windows** 10 (1809+) / 11, x64 | **Supported.** The most exercised path, and the one the installer targets. | `DCENT_Voice-Setup.exe` — per-user, no Python, no admin, no model download. |
| **Linux** (X11 and Wayland) | **Supported.** Injection is verified end-to-end on both display servers — X11 via `xclip`/`xdotool`, Wayland via `wl-clipboard`/`wtype`. Tray and overlay run under GTK/WebKitGTK. | From source with `uv` (see below). Tagged releases also build an unsigned **AppImage** and **`.deb`**. |
| **macOS** | **Incoming.** The code paths exist and are exercised in CI — NSPasteboard/CGEvent injection, an accessory-window overlay — and tagged releases build an unsigned `.dmg`/`.app`. It has not had the hardware soak-testing Windows and Linux have, so treat it as preview. | From source with `uv`, or the unsigned `.dmg` from a tagged release. |

Native release artifacts for this public beta are **unsigned** on every platform.
Windows SmartScreen and macOS Gatekeeper will warn accordingly; verify the
SHA-256 published beside each download.
...
**Linux (source).** Install the system packages first — `pynput` builds its
`evdev` extension from source, and PyGObject/pycairo need the
GObject-introspection and cairo headers:

```bash
# Debian/Ubuntu: build toolchain, audio, GTK/WebKit, tray
sudo apt install gcc linux-libc-dev pkg-config python3-dev   libportaudio2 libgirepository1.0-dev libcairo2-dev   gir1.2-gtk-3.0 gir1.2-ayatanaappindicator3-0.1
sudo apt install xclip xdotool        # X11 clipboard-paste injection
sudo apt install wl-clipboard wtype   # Wayland equivalents

uv sync
uv run dcent-voice
```

Tagged releases also publish an unsigned **AppImage** and **`.deb`** built on
Linux by [`scripts/build_linux_appimage.sh`](scripts/build_linux_appimage.sh).
Wayland injection prefers `wtype` (the virtual-keyboard protocol, no root
daemon); `ydotool` is the fallback.

**macOS (source, preview).** The macOS path is implemented and CI-tested but has
not had the hardware soak-testing the other two have:
...
macOS gates the things a dictation tool needs, so grant DCENT_Voice
**Microphone** (capture), **Accessibility** (synthesising the paste keystroke),
and **Input Monitoring** (the global push-to-talk hotkey) in System Settings →
Privacy & Security. Without Accessibility the transcript still lands on your
