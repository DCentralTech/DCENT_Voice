...
SUPPLEMENTAL_LICENSES = {
    ...
    # Linux-only GTK stack. Neither wheel carries a license file in its
    # metadata, so the bundled LGPL notice comes from packaging/licenses.
    "pygobject": ("LGPL-2.1.txt", "LGPL-2.1-or-later"),
    "pycairo": ("LGPL-2.1.txt", "LGPL-2.1-only OR MPL-1.1"),
}

#: Families where every distribution shares one license and none of the wheels
#: carry a license file. Matched on the exact name or a ``<prefix>-`` prefix, so
#: a new pyobjc framework does not silently break the release build.
SUPPLEMENTAL_LICENSE_PREFIXES: tuple[tuple[str, tuple[str, str]], ...] = (
    ("pyobjc", ("pyobjc-LICENSE.txt", "MIT")),
)


def _cpython_license_path() -> Path:
    """CPython's own license text, wherever this platform's build keeps it.

    Windows ships ``LICENSE.txt`` beside the prefix. POSIX builds — including
    the macOS framework layout the CI runners use — put it inside the standard
    library directory instead, so a single hard-coded path only ever worked on
    one of the three platforms we release for.
    """
    stdlib_name = f"python{sys.version_info.major}.{sys.version_info.minor}"
    candidates = (
        Path(sys.base_prefix) / "LICENSE.txt",
        Path(sysconfig.get_path("stdlib")) / "LICENSE.txt",
        Path(sys.base_prefix) / "lib" / stdlib_name / "LICENSE.txt",
    )
    for candidate in candidates:
        if candidate.is_file():
            return candidate
    looked = ", ".join(str(candidate) for candidate in candidates)
    raise FileNotFoundError(f"CPython license not found; looked in: {looked}")


def _supplemental_license(normalized: str) -> tuple[str, str] | None:
    """License evidence for a distribution that bundles none of its own."""
    exact = SUPPLEMENTAL_LICENSES.get(normalized)
    if exact is not None:
        return exact
    for prefix, entry in SUPPLEMENTAL_LICENSE_PREFIXES:
...
def generate(
    ...
    *,
    ...
    for dist in _embedded_distributions(toc):
        ...
        if not copied:
            supplemental = _supplemental_license(normalized)
