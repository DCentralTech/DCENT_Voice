...
def phase_launch(
    ...
    *,
    ...
    process = start_owned_process(
        ...
        cwd=str(workdir),
        env=env,
        # Launch it exactly the way a person does: no redirection at all.
        # A piped stdout hands a windowed (console=False) build a usable
        # sys.stdout that a real double-click never has, which hides every
        # crash that only happens when sys.stdout is None. Leaving all three
        # standard streams at None means Python does not set
        # STARTF_USESTDHANDLES, and DETACHED_PROCESS denies the child a
        # console, so GetStdHandle returns NULL in the child — the same
        # condition Explorer produces. CREATE_NEW_PROCESS_GROUP keeps a
        # Ctrl+C in this terminal from reaching the app under test.
        # start_owned_process still wraps it in the kill-on-close Job Object.
        stdin=None,
        stdout=None,
        stderr=None,
...
def assert_stdio_was_detached(profile_root: Path, *, timeout_s: float) -> None:
    """Prove the child really had no stdout, instead of assuming the flags worked.

    ``Popen(stdout=None, creationflags=DETACHED_PROCESS)`` is the right way to
    ask for a double-click-shaped launch, but it is not a hard guarantee: what
    the child ends up with depends on the parent's own handles and their
    inheritability. If it ever silently degrades to a usable stdout, this phase
    would go back to exercising the easy path and would stop catching the whole
    class of bug that only appears when ``sys.stdout is None`` (uvicorn's
    ``ColourizedFormatter`` calling ``isatty()``, a ``StreamHandler`` built over
    a ``None`` stream). ``util.bootlog`` records the real state on its ``boot``
    line, so assert on the app's own observation rather than on our intent.
    """
    ...
    if "stdout=none" not in boot_line:
        raise SmokeFailure(
            "the app was launched with a usable stdout, which a real double-click never has, so "
            "this run proves nothing about the windowed build. DETACHED_PROCESS did not deny the "
            f"child a console here. boot line:\n{boot_line.strip()}"
        )
